...
bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host,
    ...
    if(error){
        QIU_LOG_ERROR(g_logger) << "Address::Lookup getaddress(" << host << ","
    ...
    while(next){
        ...
        // QIU_LOG_INFO(g_logger) << ((sockaddr_in*)next->ai_addr)->sin_addr.s_addr;
        next = next->ai_next;
    }
...
bool Address::GetInterfaceAddress(std::multimap<std::string, 
    ...
    if(getifaddrs(&results) != 0){
        QIU_LOG_ERROR(g_logger) << "Address::GetInterfaceAddress getifaddrs "
    ...
    try{
        ...
        QIU_LOG_ERROR(g_logger) << "Address::GetInterfaceAddress exception";
        freeifaddrs(results);
        return false;
    }
    freeifaddrs(results);
...
IPAddress::ptr IPAddress::Create(const char* address, uint16_t port){
    ...
    if(error){
        QIU_LOG_ERROR(g_logger) << "IPAddress::Create(" << address
        << ", " << port << ") error=" << error
...
IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port){
    ...
    if(result <= 0){
        QIU_LOG_ERROR(g_logger) << "IPv4Address::Create(" << address << ", "
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port){
    ...
    if(result <= 0){
        QIU_LOG_ERROR(g_logger) << "IPv6Address::Create(" << address << ", "
        << port << ") rt=" << result << " errno=" << errno
        << " errstr=" << strerror(errno);
        return nullptr;
    }

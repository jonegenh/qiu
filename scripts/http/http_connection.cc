...
HttpConnection::~HttpConnection(){
    QIU_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
}

HttpResponse::ptr HttpConnection::recvResponse(){
    HttpResponseParser::ptr parser(new HttpResponseParser);
    uint64_t buff_size = HttpResponseParser::GetHttpResponseBuffSize();
    ...
    if(client_parser.chunked){
        ...
        do{
            ...
            QIU_LOG_INFO(g_logger) << "content_len=" << client_parser.content_len;
...
HttpResult::ptr HttpConnection::DoRequest(HttpRequest::ptr req
    ...
    if(!sock){
        ...
        , nullptr, "create socket fail: " + addr->toString() + " errno=" +
...
HttpConnection::ptr HttpConnectionPool::getConnection(){
    uint64_t now_ms = qiu::GetCurrentMs();
    ...
    if(!ptr){
        ...
        if(!addr){
            QIU_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
        Socket::ptr sock = Socket::CreateTCP(addr);
        if(!sock){
            QIU_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
            return nullptr;
        }
        if(!sock->connect(addr)){
            QIU_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
            return nullptr;
        }
    ...
    return HttpConnection::ptr(ptr, 
        std::bind(&HttpConnectionPool::ReleasePtr, std::placeholders::_1, this));
}

void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool){
    ++ptr->m_request;
    if((!ptr->isConnected()) 
    || ((ptr->m_createTime + pool->m_maxAliveTime) >= qiu::GetCurrentMs())

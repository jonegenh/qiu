...
void LogAppender::setFormatter(LogFormatter::ptr val){
    ...
    m_formatter = val;
    if(m_formatter){
        m_hashFormatter = true;
    }else{
        m_hashFormatter = false;
    }
}
...
void Logger::setFormatter(LogFormatter::ptr val){
    ...
    for(auto& i : m_appenders){
        ...
        if(!i->m_hashFormatter){
            i->m_formatter = m_formatter;
        }
    }
}
void Logger::setFormatter(const std::string& val){
    qiu::LogFormatter::ptr new_val(new qiu::LogFormatter(val));
    if(new_val->isError()){
...
std::string FileLogAppender::toYamlString(){
    ...
    if(m_hashFormatter && m_formatter){
        node["formatter"] = m_formatter->getPattern();
    }
    std::stringstream ss;
    ss << node;
...
qiu::ConfigVar<std::set<LogDefine> >::ptr g_log_defines = 
    qiu::Config::Lookup("logs", std::set<LogDefine>(), "logs config");
...
struct LogIniter{
    LogIniter(){
        ...
        const std::set<LogDefine>& new_value){
            //新增
            QIU_LOG_INFO(QIU_LOG_ROOT()) << "on_logger_conf_changed";
            for(auto& i : new_value){
                auto it = old_value.find(i);
                qiu::Logger::ptr logger;
                if(it == old_value.end()){
                    //新增logger
                    logger = QIU_LOG_NAME(i.name);
                }else{
                    if(!(i == *it)){
                        //修改的logger
                        logger = QIU_LOG_NAME(i.name);
            ...
            for(auto& i : old_value){
                ...
                if(it == new_value.end()){
                    auto logger = QIU_LOG_NAME(i.name);
